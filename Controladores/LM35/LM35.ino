...
mPID MyPID(-38, -3, -0.1, 1, 1);                                          // Chamada do construtor para o cooler
                                         // Chamada do construtor para a lâmpada

void setup() {
  
  Serial.begin(9600);
  pinMode(LM35,INPUT);
  MyPID.setSetPoint(setPoint);                                              // Definição de setpoint para o cooler                                          // Definição de setpoint para a lâmpada
...
void loop() {
  ...
  if (timeChange>=Ts){
    ...
    if((millis() >= (400000*0.6)) && (millis() <= (400000*0.61)))
      ...
      output1 = 160;
    else if (millis() >= (400000*4.4) && (millis() <= (400000*4.63)))
      output1 = 100;
    else
      output1 = output1;
